...
class PreprocessingMixin:
    ...
    @staticmethod
    def _encode_text_feature(text_feat, model):
        try:
            model_name_or_path = get_model_path(model, None)
            model = LLM(model=model_name_or_path, task="embed")
            outputs = model.embed(text_feat)
            embeddings = torch.tensor([o.outputs.embedding for o in outputs]).cpu()
        except Exception:
            print("input model is not LLM, using SentenceTransformer")
